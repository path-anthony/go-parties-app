...
**What we decided:** A package on the storefront is its items, nothing more: the cart carries each item once and the booking holds one unit per item, so a package quantity above 1 and the package's own bundle price are not carried into the booking yet (the admin's Booking has no package field). No gate in front of the product; sign in is a door, never a step. Value props that the screen already makes (a real package in seconds, the Ask GO card) are cut rather than repeated; the ones about dates and delivery ride the captions. The deploy fix lives in the repo (`vercel.json`), not in a dashboard setting, so it survives a rebuild.
