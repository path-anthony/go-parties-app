...
- **Skill**: a crew skill, managed in Settings (add, rename, delete). Twenty today: the original eight (DJ/MC, Photographer, Videographer, Photo Booth Attendant, Day-of Coordinator, Waitstaff, Bartender, Live Musician) and twelve for specialist performers and services. Item.skills, CrewMember.skills and Gig.skill hold the names as text; a rename rewrites them everywhere in one transaction, and a skill can't be deleted while an item or a crew member lists it.
...
- **Package**: a curated bundle of real items with one manual price (not a sum; the gap is Andy's discount). Name, description, price, Draft or Published, one or more occasions (storefront sub-occasion strings; the public feed lists a package under each of them), search keywords (short customer-facing terms, suggested by Claude from the package's name, occasions and items, edited by the admin before saving), a photo. Only Published packages are public, and publishing requires at least one occasion and at least one item. The old single occasion and free-text theme tag became these two lists on 2026-09-24.
...
Not in the schema: **Theme** (the package theme tag is gone; search keywords replaced it), **Client**, **Contract**, **Payment**, and a rate on a gig or a crew member (payouts stay off-platform for now). The storefront's original static package flow (`/book/*`, `/held`) was removed on 2026-09-22; every booking a customer can make now goes through the real checkout. The static catalog in `src/data/catalog.ts` survives only as sample data for the `/kit` design page.
