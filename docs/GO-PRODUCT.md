...
The architecture spine for The GO Event Group booking platform. This file changes only when the architecture changes. It does not change for UI tweaks, copy edits, or styling. Last architecture change: 2026-09-23 (Crew and Gigs: service items are booked against people, not units). Rewritten 2026-09-22 from the two repos and the live database, not from earlier drafts.
...
- **Item**: anything sellable. The menu. Name, category, nullable price (TBD and custom-quote items exist and are excluded from Ask GO's math), price unit as free text, internal notes, photo, and an optional `required_skill`. Uploaded photos are stored inline as data URLs in `photo_url`. An item with a required skill is a **service item**: a person's time (a DJ, a photographer), never a physical piece. It is never given units, and the API refuses units on it and a skill on anything with units, so no item is ever both.
- **Unit**: one physical instance of a physical item. The warehouse. A label, a manual status (Available, Booked, Maintenance), and a booking calendar through BookingUnit. Optional per item, and still the gap for physical inventory: only Snow Cone Station (2 units) and two test items have any.
...
- **Service items are promised against people, not units.** A service item is free on a date when the active crew members with its skill outnumber the gigs already needing that skill that day. Inside the booking transaction the crew rows for the skill are locked (`FOR UPDATE`, waiting rather than skipping, since a gig isn't tied to a specific person at booking time) before the gigs are counted, so two bookings racing for the last free DJ take turns and exactly one wins. A reschedule re-checks every gig's skill on the new date under the same lock and refuses the whole move if the crew can't cover it; adding a service item from the admin's booking popup goes through the same check. The public catalog and availability report the free count for a service item the same way they report free units for a physical one.
